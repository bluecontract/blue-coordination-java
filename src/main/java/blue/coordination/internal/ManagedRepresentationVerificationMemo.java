...
/** Complete pure proof reuse, owned by a resident store or a verified immutable publication entry. */
final class ManagedRepresentationVerificationMemo {
    ...
    // A process entry can be shared by independent stores. Pure proofs run outside this monitor.
    synchronized ManagedRepresentationTransition find(Request key) { return entries.get(key); }

    synchronized ManagedRepresentationTransition retain(Request key, ManagedRepresentationTransition proved) {
    ...
    synchronized void clear() { entries.clear(); retainedWeight = 0L; }
    ...
    private static long weight(Request key, ManagedRepresentationTransition value) {
        ...
        // are already retained by the same resident publication inventory, or by the explicitly
        // bound cold receipt scope whose eviction epoch retires this memo before its next use,
        // or the process entry that owns both the complete receipt and this budgeted memo.
        // Staged proposals must never enter this memo: that lifetime argument does not apply.
    ...
    static final class Request {
        ...
        ManagedRepresentationTransition prove() {
            var proved = new ManagedRepresentationTransition(document, epoch, anchor, predecessor, input, result, receipt);
            if (proved.rootedCheckpointReferenceProofIdentity().isPresent()) {
                var rooted = publication.rootedTerminalEvidence();
                if (rooted == null) throw new IllegalArgumentException("Original rooted representation authority is unavailable");
                rooted.requireCheckpointReferencePosition(result, ContractsClosureAdapter.coordinationId(document),
