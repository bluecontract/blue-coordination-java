 ...
 */
public final class CoordinationRuntimeGasSupport {
    ...
    private static final Map<String, Long> WEIGHTS =
            loadWeights();
    private static final Map<Object, Integer> NEXT_SEQUENCE =
            new WeakHashMap<Object, Integer>();
    private static final Map<Object, WeakReference<ActiveLedger>>
            ACTIVE_LEDGERS =
            new WeakHashMap<
                    Object,
                    WeakReference<ActiveLedger>>();

    private CoordinationRuntimeGasSupport() {
    }
    ...
    public static Ledger open(RuntimeWorkSession session) {
        ...
        return acquire(exact, exact, null);
    }

    /**
     * Opens one live Coordination ledger through the public Contracts
     * processor capability.
     *
     * <p>The raw {@link RuntimeWorkSession} is processor-internal in the
     * modular Language API. Hosted Coordination processors therefore use this
     * overload so every charge still enters the invocation's canonical work
     * session exactly once without reaching through a split package.</p>
     *
     * @param context live processor-owned invocation context
     * @return live ledger that must be submitted exactly once
     */
    public static Ledger open(ProcessorExecutionContext context) {
        ProcessorExecutionContext exact =
                Objects.requireNonNull(context, "context");
        return acquire(exact, null, exact);
    }
    ...
    /** Public-context counterpart of {@link #inComponent(RuntimeWorkSession, ComponentWork)}. */
    public static <T> T inComponent(
            ProcessorExecutionContext context,
            ComponentWork<T> work) {
        Objects.requireNonNull(work, "work");
        Ledger ledger = open(context);
        Throwable failure = null;
        try {
            return work.run();
        } catch (RuntimeException | Error exception) {
    ...
    /**
     * Charges one isolated unit through the public Contracts processor
     * capability.
     */
    public static void charge(
            ProcessorExecutionContext context,
        ...
        boolean submitted = false;
        try {
            ledger.charge(counter, quantity, gasContext);
            ledger.submit();
            submitted = true;
        } finally {
            if (!submitted) {
    ...
    private static synchronized int nextSequence(
            Object owner) {
        Integer current = NEXT_SEQUENCE.get(owner);
    ...
    private static synchronized Ledger acquire(
            Object invocationOwner,
            RuntimeWorkSession session,
            ProcessorExecutionContext context) {
        WeakReference<ActiveLedger> reference =
                ACTIVE_LEDGERS.get(invocationOwner);
        ...
        if (active != null
                && (active.submitted
                || active.handles == 0)) {
            ACTIVE_LEDGERS.remove(invocationOwner);
            active = null;
        }
        ...
        Thread threadOwner = Thread.currentThread();
        if (active != null && active.owner != threadOwner) {
            throw new IllegalStateException(
                    "Concurrent Coordination runtime ledger ownership is not "
                            + "supported for one work session");
        }
        if (active == null) {
            int sequence = nextSequence(invocationOwner);
    ...
    private static synchronized void submit(
        ...
        if (lastHandle) {
            removeActive(handle.owner, active);
        }
        if (active.abandoned) {
            throw new IllegalStateException(
        ...
        try {
            if (handle.session != null) {
                ...
                handle.context.submitRuntimeGasLedger(active.ledger);
            }
    ...
    private static void removeActive(
        ...
        WeakReference<ActiveLedger> reference =
                ACTIVE_LEDGERS.get(owner);
        if (reference == null
                || reference.get() == expected) {
            ACTIVE_LEDGERS.remove(owner);
        }
    }

    private static Map<String, Long> loadWeights() {
        InputStream input = CoordinationRuntimeGasSupport.class
                .getClassLoader()
    ...
    public static final class Ledger {
        ...
        public void submit() {
            CoordinationRuntimeGasSupport.submit(this);
        }
        ...
        public boolean isSessionOpen() {
            ...
            /*
             * ProcessorExecutionContext exposes no public liveness probe.
             * Requiring its semantic-output capability as a proxy is wrong:
             * non-output workflow phases legitimately have no such
             * capability.  Public-context ledgers are owned and submitted
             * synchronously inside the processor callback, before the host
             * closes the context; submitRuntimeGasLedger remains the
             * authoritative fail-closed liveness check.
             */
            return true;
        }

        private void abandon() {
            CoordinationRuntimeGasSupport.abandon(this);
        }
    ...
    public interface ComponentWork<T> {
        T run();
    }
