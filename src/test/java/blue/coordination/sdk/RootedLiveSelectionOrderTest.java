...
/** LIVE selection preserves independent work, retained ordering, and exact histories. */
final class RootedLiveSelectionOrderTest {
    @Test void earlierNoMatchAndRejectedTerminalInputsDoNotHideTheNextLiveInput() throws Exception {
        ...
        try (var f = new RootedSdkFixture()) {
            ...
            // when
            var selected = f.control.nextLiveInput(source.id());
            // then
            ...
            assertEquals(live.blueId(), f.control.nextLiveInput(source.id()).orElseThrow());
    ...
    @Test void anEqualRetainedOrderPreservesItsIndependentLiveInput() throws Exception {
        // given
    ...
    private static List<String> verifyRetainedOrdering(boolean equalSourceOrder) throws Exception {
        try (var f = new RootedSdkFixture()) {
            ...
            if (!equalSourceOrder) {
                ...
                // Equal source order does not consume the independent root's LIVE input.
                assertEquals(live.blueId(), f.control.nextLiveInput(independent.id()).orElseThrow());
            }
