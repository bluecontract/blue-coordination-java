...
| 13.11 | Multi-child barrier, canonical interleaving, and no overtaking | `SdkManagedCatchUpBarrierAndLiveExtensionTest.multiChildBarrierExtendsBeforeReadyAndPreventsDirectOvertaking`; `ManagedCatchUpPlannerTest.waitingOrBlockedSiblingGatesOnlyItsImpactedConsumer` | One barrier owns B/C plans; receipts interleave by original source order; WAITING/BLOCKED siblings gate only their consumer; later direct A entry remains queued until completion; unrelated consumers continue | **SOURCE-COVERED** | **PENDING ROOT RUN** |
| 13.12 | Live source extension before ready promotion | `SdkManagedCatchUpBarrierAndLiveExtensionTest.multiChildBarrierExtendsBeforeReadyAndPreventsDirectOvertaking`; fairness guard in `SdkManagedCatchUpBarrierAndLiveExtensionTest.boundedDrainsFairlyAdvanceContinuingSourceTrafficAndCatchUp` | Genuine source commits extend the frontier atomically; retained external/managed turns advance both frontier and cursor under repeated `(1,1)` drains, survive coordinator restart, block direct overtaking, and settle after finite input; continuing input remains `CATCHING_UP` | **SOURCE-COVERED**; **SOURCE-CAVEAT**: the first executable fixture uses the isomorphic B3 frontier rather than the prompt's literal B6 label | **PENDING ROOT RUN** |
