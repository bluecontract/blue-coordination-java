...
Rooted checkpoint processing, introduced in RC7, lets a selected root calculate
its required forward dependencies and publish only its verified owned changes. Historical
...
terminal failure leaves no partial document or topology mutation. In this candidate,
...
The rc.13 release requires verification through the Maven-Central-only
artifact lane:
...
The public rc.13 release lane uses the published-artifact mode by default. An
...
JReleaser remain restricted to the published rc.13 lane.
...
The rc.13 release workflow runs the same gates, stages signed artifacts,
...
[rc.13 release decision](docs/releases/3.0.0-rc.13.md).
...
[3.0.0-rc.13 decision](docs/releases/3.0.0-rc.13.md). The documents below are
...
- [3.0.0-rc.13 release decision](docs/releases/3.0.0-rc.13.md)
