...
const RELEASE_AUTHORITY = 'docs/releases/3.0.0-rc.10.md';
...
function authorityRelease(content) {
  const match = content.match(/^RC\d+_VERSION:\s*(\S+)\s*$/m);
  if (!match) {
    throw new Error(`Release authority is missing RC version: ${RELEASE_AUTHORITY}`);
  }
  return match[1];
}
