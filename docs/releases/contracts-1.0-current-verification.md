...
This tree prepares the `3.0.0-rc.11` bounded external-pilot candidate.
...
All current release verification resolves Language `3.1.0-rc.27`, BEX
...
published dependencies. `verifyRcReadiness` adds exact rc.11 policy and graph
checks and writes fresh built-artifact hashes to
`build/reports/release/3.0.0-rc.11-readiness.json`.

The complete current release gate and focused rc.11 capability inventory
...
The maintainability guardrails remain at most 253 production Java files,
74,084 production Java lines, and 87 public source types across
