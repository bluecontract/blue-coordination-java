...
`blueDependencyMode=published-artifact` is the rc.9 build and release default.
...
release authority for rc.9. The development lane is valid
...
For rc.9, verify fresh remote availability and the
...
RC9 uses the Maven-Central-only release gate:
...
evidence, but does not itself publish rc.9 or make it production-ready.
...
`verifyRcReadiness` is the rc.9 release-readiness gate:
...
The task includes `releaseCheck` and `dependencyPreflight`, validates the rc.9
release authority and explicit non-claims, then records the
fresh artifact hashes in
`build/reports/release/3.0.0-rc.9-readiness.json`. It validates version
`3.0.0-rc.9`, the published-artifact lane, and the focused rc.9 capability
inventory in addition to the complete current suite.

The Build workflow prepares and seals the exact rc.9 version in its isolated
...
For a downstream development handoff after the rc.9 gates pass on a clean
...
[3.0.0-rc.9 decision](../releases/3.0.0-rc.9.md).
