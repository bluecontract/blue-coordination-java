...
`3.0.0-rc.10` is authorized as a bounded external-pilot release candidate
...
[rc.10 release decision](../releases/3.0.0-rc.10.md).
...
The complete current release gate and focused rc.10 capability inventory are
...
workflow prepares rc.10 and seals a local verification commit before running
...
4. for RCs, prepares the version authorized by `docs/releases/3.0.0-rc.10.md`;
...
8. runs the complete Java 21 `releaseCheck` and Java 17 `stageRelease`
   concurrently; staging includes the complete release and rc.10 gates;
...
performance exception is rc.1-specific and is not part of rc.10 or any future
stable release.
