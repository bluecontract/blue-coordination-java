...
`3.0.0-rc.15` is authorized as a bounded external-pilot release candidate
...
[rc.15 release decision](../releases/3.0.0-rc.15.md).
...
The complete current release gate and focused rc.15 capability inventory are
...
Keep `.cz.toml` at the authorized, not-yet-tagged candidate (`3.0.0-rc.15`)
...
owns the final version commit and tag; it retains RC15 while its tag is absent
...
2. validates credentials and prepares the RC authorized by
   `docs/releases/3.0.0-rc.15.md`, with a local verification commit and tag;
...
5. requires all four jobs to succeed, validates complete test inventory, topology,
   ...
   release and RC15 readiness gates and stages the Java 17 artifacts;
...
bounds, or a stable latency SLA. See the RC15 decision for the exact storage
...
performance exception is rc.1-specific and is not part of rc.15 or any future
stable release.
