...
`3.0.0-rc.11` is authorized as a bounded external-pilot release candidate
...
workflow prepares rc.11 and seals a local verification commit before running
...
4. for RCs, prepares the version authorized by `docs/releases/3.0.0-rc.11.md`;
...
8. runs the complete Java 21 `releaseCheck` and Java 17 `stageRelease`
   concurrently; staging includes the complete release and rc.11 gates;
...
performance exception is rc.1-specific and is not part of rc.11 or any future
stable release.
