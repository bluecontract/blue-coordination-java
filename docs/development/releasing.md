...
`3.0.0-rc.6` is authorized as a bounded external-pilot release candidate
...
[rc.6 release decision](../releases/3.0.0-rc.6.md).

The release consumes only Maven Central artifacts:

| Component | Version |
| --- | --- |
| Language | `3.1.0-rc.24` |
| BEX core/contracts | `1.1.0-rc.5` |
| Repository | `3.0.0-rc.22` |
| Coordination | `3.0.0-rc.6` |

Coordination directly owns the complete Language rc.24 graph. BEX and
Repository Language transitive edges remain excluded, and the generated POM
publishes the same exclusions.

The complete current release gate and focused rc.6 capability inventory are
the required semantic evidence for this release. Their documentation does not
claim that they have passed. Historical rc.4 and rc.5 receipts remain evidence
...
workflow prepares rc.6 and seals a local verification commit before running
...
4. prepares the version authorized by `docs/releases/3.0.0-rc.6.md`;
...
8. runs `stageRelease` on Java 17, including the complete release and rc.6
   gates;
...
performance exception is rc.1-specific and is not part of rc.6 or any future
stable release.
