...
| [Exact-cutoff Journal fairness](rooted-journal-cutoff-fairness.md) | B's T200 attachment imports A1; A2 remains pending while unrelated C has only submitted T300. Global audit keeps choosing C's LIVE lane; bounded T200 calls refuse retained work and pause forever. | A bounded Journal-only call explicitly yields to its eligible retained lane without executing it, crossing its cutoff, clearing failure isolation or changing history/gas. The isolated regression reproduces the bug; the narrow two-file correction and adjacent/restart controls await qualification. No public API change. |
